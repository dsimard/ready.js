...
var r = {
  ...
  loadConfig : function(extConfig) {
    ...
    r.config.dest = argv.d || argv.dest || r.config.src;
      ...
      if (!err && stats.isFile()) {
        ...
        if (__dirname.match(/\.npm/)) {
          ...
          u.pump(is, os, function() {
            r.logger.log("Compiler successfully installed to '" + installPath + "'");
          });
        } else {
          r.logger.error("Installing a compiler is available only with a NPM installation. Run `npm install ready`.");
        }
      } else {
        r.logger.error(["'", path, "' is not valid file for a compiler"].join(""));
      }
    });
  }
}
